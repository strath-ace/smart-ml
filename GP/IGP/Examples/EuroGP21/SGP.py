'''SGP: code to create offline the GP control law using the Standard Genetic Programming
...
if __name__ == '__main__':
    try:
        os.mkdir('Results')
        os.mkdir('Results/Mod_SGP_{}'.format(bench))
    ...
    while n < ntot:
        ...
        if terminals == 1:
            ...
            y_best_train = f_best(input_true[0,:], input_true[1,:], input_true[2,:], input_true[3,:], input_true[4,:], input_true[5,:], input_true[6,:], input_true[7,:])
            y_best_test = f_best(input_test[0, :], input_test[1, :], input_test[2, :], input_test[3, :], input_test[4, :], input_test[5, :], input_test[6, :], input_test[7, :])
        ...
        np.save("Results/Mod_SGP_{}/{}_Mod_SGP_{}_POP_STATS".format(bench, n, bench), pop_statistics)
        np.save("Results/Mod_SGP_{}/{}_Mod_SGP_{}_IND_LENGTHS".format(bench, n, bench), ind_lengths)
        np.save("Results/Mod_SGP_{}/{}_Mod_SGP_{}_GEN".format(bench, n, bench), log.select("gen"))
        np.save("Results/Mod_SGP_{}/{}_Mod_SGP_{}_FIT".format(bench, n, bench),
                np.array(log.chapters['fitness'].select('min')))
        to_save = np.vstack((to_save, [t_offdesign, RMSE_train, RMSE_test]))
        n += 1

    np.save("Results/Mod_SGP_{}/{}_Mod_SGP_{}_T_RMSE".format(bench, n, bench), to_save)

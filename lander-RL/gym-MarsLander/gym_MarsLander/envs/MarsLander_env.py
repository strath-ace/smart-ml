...
class MarsLanderEnv(gym.Env, EzPickle):
	metadata = {
	...
	def __init__(self, step_max=2000, env_params=None):
		...
		self.world = Box2D.b2World(gravity=(0,-3.72))
		self.moon = None
		self.lander = None
		self.particles = []
	...
	def reset(self):
		...
		# initial_x = 3.0*VIEWPORT_W/SCALE/4.0
	...
	def step(self, action):
		...
		# Adding a crosswind element
		if self.env_params:
			impulse_pos = (self.lander.position[0], self.lander.position[1])
			f_wind = self.env_params['wind'] + (np.random.rand(1)-0.5)*2*self.env_params['gust']
			self.lander.ApplyLinearImpulse( (f_wind, 0), impulse_pos, True)
...
class MarsLanderContinuous(MarsLanderEnv):
	continuous = True
...
def exp_heuristic_lander(env, N_ep=5, seed=None, render=False):
	exp_heuristic = []
...
if __name__ == '__main__':
	demo_heuristic_lander(MarsLanderEnv(), render=True)
